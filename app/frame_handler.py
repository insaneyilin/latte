...
class FrameHandler():
    ...
    def __init__(self, data_dir):
        ...
        self.POINTCLOUD_BIN_DIR = join(self.DATA_DIR, 'bin_data')
        self.ANNO_JSON_DIR = join(self.DATA_DIR, 'anno')
        drive = data_dir.split('/')[-1]
        self.drives[drive] = []
        for f in listdir(self.POINTCLOUD_BIN_DIR):
            if isfile(join(self.POINTCLOUD_BIN_DIR, f)) and '.bin' in f:
    ...
    def get_pointcloud(self, drivename, fname, dtype=str, ground_removed=False):
        ...
        """
        filename = join(self.POINTCLOUD_BIN_DIR, fname.split(".")[0] + ".bin")
    ...
    def load_annotation(self, drivename, fname, dtype='object'):
        ...
        try:
            with open(join(self.ANNO_JSON_DIR, fname), "r") as read_file:
            ...
            print('FrameHandler load_annotation failed.')
            return ""
        # try:
